...
export default class NewsCard {
  ...
  _save() {
    if (!this.icon.classList.contains('result-card__button-icon_marced')) {
      myNewsExplorerApi.createArticle(this.keyWord, this.title, this.description,
        this.data, this.source, this.url, this.urlToImage)
        .then((data) => {
          this.renderIcon(true);
          this.cardElement.setAttribute('id', data._id);
        })
        .catch((err) => {
          alert('Не удалось сохранить статью. Ошибка: ' + err);
        });
    } else {
      myNewsExplorerApi.removeArticle(this.cardElement.getAttribute('id'))
        .then(() => {
